  ...
  initiate: (renderId, elementObj) => {
    ...
    if (keys.includes("elements") && elementObj["elements"].length > 0) {
      ...
      if (keys.includes("element")) {
        ...
        // id
        if (keys.includes("id"))
          parentElement.setAttribute("id", elementObj["id"]);
        ...
        // onclick
        if (keys.includes("onclick"))
          parentElement.setAttribute("onclick", elementObj["onclick"]);

        // text
        if (keys.includes("text")) parentElement.innerText = elementObj["text"];

        // signText
        if (keys.includes("signText"))
          parentElement.innerHTML = elementObj["signText"];

        // src
        if (keys.includes("src"))
          parentElement.setAttribute("src", elementObj["src"]);

        // placeholder
        if (keys.includes("placeholder"))
          parentElement.setAttribute("placeholder", elementObj["placeholder"]);

        // value
        if (keys.includes("value"))
          parentElement.setAttribute("value", elementObj["value"]);

        // name
        if (keys.includes("name"))
          parentElement.setAttribute("name", elementObj["name"]);

        // type
        if (keys.includes("type"))
          parentElement.setAttribute("type", elementObj["type"]);

        // rows
        if (keys.includes("rows"))
          parentElement.setAttribute("rows", elementObj["rows"]);

        // ariaHidden
        if (keys.includes("ariaHidden"))
          parentElement.setAttribute("aria-hidden", elementObj["ariaHidden"]);

        // inline styles
        if (keys.includes("inlineStyles"))
          render.applyInLineStyles(elementObj["inlineStyles"], parentElement);

        // children rendering
        if (keys.includes("children")) {
          elementObj["children"].elements.forEach((element) =>
            render.initiate(parentElement, element)
          );
        }
